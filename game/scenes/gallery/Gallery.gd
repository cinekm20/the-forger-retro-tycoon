...
func _ready() -> void:
	...
	var bodyguard_btn := ScreenHelpers.make_button(
		root,
		"Zatrudnij ochroniarza (%.0f M)" % Security.BODYGUARD_COST,
	...
	for rival in AIPlayers.rivals:
		...
		rival_label.text = "%s: %d obrazów" % [rival["name"], AIPlayers.get_rival_painting_count(rival["id"])]
		rival_row.add_child(rival_label)
		var gangster_btn := Button.new()
		gangster_btn.text = "Wyślij gangstera (%.0f M)" % Security.GANGSTER_COST
...
func _on_send_gangster_pressed(rival_id: String) -> void:
	...
	security_label.text = (
		"Gangster wraca z obrazem!" if success else "Gangster wraca z pustymi rękami — pieniądze przepadły."
	)


func _update_security_label() -> void:
	security_label.text = (
		"Masz ochroniarza — obrazy chronione przed kradzieżą."
		if Security.has_bodyguard
		else "Bez ochrony — Twoja kolekcja jest narażona na kradzież (~%.0f%% szans/tydzień)." % [
