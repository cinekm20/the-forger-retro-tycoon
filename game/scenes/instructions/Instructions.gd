...
func _ready() -> void:
	...
	_add_section(root, tr("Zwycięstwo i porażka"), [], [
		tr("Wygrywasz, gdy zdobędziesz wszystkie obrazy z głównego katalogu — 40 na trzech trudniejszych poziomach, 15 na dwóch najłatwiejszych. Przegrywasz, jeśli przez 60 kolejnych dni gry masz ujemną gotówkę (bankructwo) albo jeśli rywal skompletuje kolekcję pierwszy. W grze wieloosobowej (hot-seat, do 4 graczy) wygrywa ten gracz, który jako pierwszy skompletuje kolekcję."),
		tr("Poziom trudności (wybierany raz, przy zakładaniu nowej gry, wspólny dla wszystkich graczy) to więcej niż sam próg zwycięstwa — steruje też, jak często zdarzają się negatywne losowe zdarzenia (susza, zamieszki, kradzieże, krachy giełdowe, reforma walutowa) i jak dotkliwe są ich skutki, a także ile plonu dają plantacje: od Bardzo łatwego (zero takiego ryzyka, poczwórny plon) po Bardzo trudny (pełne ryzyko, ale wciąż półtora raza więcej plonu niż dawniej)."),
	])
